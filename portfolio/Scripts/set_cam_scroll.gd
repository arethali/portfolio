...
func _ready():
	...
	cam_scroll.custom_minimum_size = min_size
	cam_scroll.size = min_size
	cam_scroll.get_v_scroll_bar().modulate.a = 0
	cam_scroll.position = panel_top_left_corner + Vector2(padding_sides.x , padding_vert.x) 
